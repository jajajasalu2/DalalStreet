...
class TransactionController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if ($request->input('buy_sell') == 1) {
            $error_code = ControllerScopes::buy_share(
                $request->input('team_id'),
                $request->input('company_id'),
                $request->input('amount')
            );
        } else if ($request->input('buy_sell') == 2) {
            $error_code = ControllerScopes::sell_share(
                $request->input('team_id'),
                $request->input('company_id'),
                $request->input('amount')
            );
        } else if ($request->input('buy_sell') == 3) {
            $error_code = ControllerScopes::buy_back(
                $request->input('team_id'),
                $request->input('company_id'),
                $request->input('amount')
            );
        } else if ($request->input('buy_sell') == 4) {
            $error_code = ControllerScopes::short_sell(
                $request->input('team_id'),
                $request->input('company_id'),
                $request->input('amount')
            );
        ...
        $error_code = ControllerScopes::adjust_rate(
            $request->input('amount'),
            $request->input('company_id'),
